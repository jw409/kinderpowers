...
**Measuring compression.** The ratio is reproducible, not hand-waved. `scripts/bench_tokens.py` prints a per-endpoint token table (tiktoken, `gh api` as the raw baseline, public `cli/cli`), and the `test_compression_floors` live test enforces per-endpoint byte-ratio floors so a regression fails CI. Measured: issues_list 6.4×, prs_list 14.6×, prs_get 9.1× (~11× blended across read endpoints); `branch_status` ~1200×.
