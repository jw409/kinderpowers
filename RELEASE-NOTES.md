...
The underlying skills (`kinderpowers:brainstorming`, `kinderpowers:executing-plans`, `kinderpowers:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
- Updated slash commands to use fully qualified skill names (e.g., `kinderpowers:brainstorming`)
...
- Namespaced skills: `kinderpowers:skill-name` for superpowers skills, `skill-name` for personal
...
- Added `kinderpowers:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `kinderpowers:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `kinderpowers:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `kinderpowers:code-reviewer`
...
- All internal skill references now use `kinderpowers:` namespace prefix
- Updated format: `kinderpowers:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/kinderpowers:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
