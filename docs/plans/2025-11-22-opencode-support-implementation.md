 ...
 *
 * @param {string} skillName - Name like "kinderpowers:brainstorming" or "my-skill"
 * @param {string} superpowersDir - Path to superpowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, superpowersDir, personalDir) {
    // Strip kinderpowers: prefix if present
    const forceSuperpowers = skillName.startsWith('kinderpowers:');
    const actualSkillName = forceSuperpowers ? skillName.replace(/^kinderpowers:/, '') : skillName;

    // Try personal skills first (unless explicitly kinderpowers:)
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "kinderpowers:brainstorming" or "my-custom-skill")')
        }),
...
```javascript
      {
        ...
        execute: async () => {
          ...
          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'kinderpowers:';
...
**Skills naming:**
- Superpowers skills: \`kinderpowers:skill-name\` (from ~/.config/opencode/superpowers/skills/)
...
**Below is the full content of your 'kinderpowers:using-superpowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
```
use use_skill tool with skill_name: "kinderpowers:brainstorming"
```
...
Run: `.codex/superpowers-codex use-skill kinderpowers:brainstorming | head -20`
