...
```bash
INIT=$(node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" init milestone-op)
...
node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" phases list
```
...
PHASE_INFO=$(node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" find-phase 01 --raw)
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
```bash
NYQUIST_CONFIG=$(node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" config get workflow.nyquist_validation --raw 2>/dev/null)
```
