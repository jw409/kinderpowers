...
```bash
INIT=$(node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" init execute-phase "${PHASE_ARG}")
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `${CLAUDE_PLUGIN_ROOT}/gsd/workflows/execute-plan.md` **inline**
...
```bash
PLAN_INDEX=$(node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
5b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_CHAIN=$(node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
```bash
PARENT_INFO=$(node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" commit "docs(phase-${PARENT_PHASE}): resolve UAT gaps and debug sessions after ${PHASE_NUMBER} gap closure" --files .planning/phases/*${PARENT_PHASE}*/*-UAT.md .planning/debug/resolved/*.md
```
</step>
...
```bash
COMPLETION=$(node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" commit "docs(phase-{X}): complete phase execution" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md {phase_dir}/*-VERIFICATION.md
```
</step>
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" commit "docs(phase-{X}): evolve PROJECT.md after phase completion" --files .planning/PROJECT.md
```
...
Read and follow `${CLAUDE_PLUGIN_ROOT}/gsd/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
