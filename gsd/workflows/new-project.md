...
```bash
INIT=$(node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" init new-project)
...
```bash
mkdir -p .planning
node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" commit "chore: add project config" --files .planning/config.json
```
...
```bash
mkdir -p .planning
node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" commit "docs: initialize project" --files .planning/PROJECT.md
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs" commit "docs: define v1 requirements" --files .planning/REQUIREMENTS.md
```
