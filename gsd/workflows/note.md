...
- **Global scope**: `/home/jw/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/jw/.claude/notes/`)
...
2. Glob `/home/jw/.claude/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `/home/jw/.claude/notes/` — works in any directory
