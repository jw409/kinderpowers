...
Key references:
- @${CLAUDE_PLUGIN_ROOT}/gsd/references/ui-brand.md (display patterns)
- @${CLAUDE_PLUGIN_ROOT}/gsd/agents/gsd-user-profiler.md (profiler agent definition)
- @${CLAUDE_PLUGIN_ROOT}/gsd/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="${CLAUDE_PLUGIN_ROOT}/gsd/USER-PROFILE.md"
...
```bash
cp "${CLAUDE_PLUGIN_ROOT}/gsd/USER-PROFILE.md" "${CLAUDE_PLUGIN_ROOT}/gsd/USER-PROFILE.backup.md"
```
...
✓ Stores profile at ${CLAUDE_PLUGIN_ROOT}/gsd/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node ${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node ${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `${CLAUDE_PLUGIN_ROOT}/gsd/references/user-profiling.md`
...
Reference: @${CLAUDE_PLUGIN_ROOT}/gsd/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node ${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node ${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to ${CLAUDE_PLUGIN_ROOT}/gsd/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  ...
  - "Global CLAUDE.md" -- "Add profile to /home/jw/.claude/CLAUDE.md for all projects"

**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at ${CLAUDE_PLUGIN_ROOT}/gsd/USER-PROFILE.md" and jump to step 10.
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /gsd:dev-preferences at ~/.claude/commands/gsd/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
node ${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/gsd/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to /home/jw/.claude/CLAUDE.md"
...
```bash
BACKUP_PATH="${CLAUDE_PLUGIN_ROOT}/gsd/USER-PROFILE.backup.md"
```
...
Your profile:    ${CLAUDE_PLUGIN_ROOT}/gsd/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /gsd:dev-preferences   ~/.claude/commands/gsd/dev-preferences.md
  ✓ CLAUDE.md section       ./CLAUDE.md
  ✓ Global CLAUDE.md        /home/jw/.claude/CLAUDE.md
```
