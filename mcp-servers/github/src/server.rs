...
/// Hard ceiling on how long any single MCP tool call may run before the
/// server returns an error instead of leaving the caller hanging.
///
/// The client (`client.rs`) already bounds each individual GitHub API call at
/// `OVERALL_CALL_TIMEOUT` (90s), but a tool handler can chain several API
/// calls (e.g. `files_push` does one request per blob, the GraphQL draft
/// toggle does a GET then a mutation) — so the per-call budget alone does not
/// bound a tool. This wraps the *entire* handler dispatch, making "the caller
/// never waits longer than this" a property of the server regardless of how
/// many internal requests a tool makes. Set above the per-call budget so a
/// legitimately slow single request still completes.
const GLOBAL_TOOL_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(150);
...
pub struct FileCreateOrUpdateParams {
    ...
    pub branch: String,
    /// Full 40-char blob SHA of existing file (required for updates, omit for
...
pub struct FileDeleteParams {
    ...
    pub branch: String,
    /// Full 40-char blob SHA of the file being deleted. Abbreviated SHAs are
...
pub struct BranchCreateParams {
    ...
    pub branch: String,
    /// Full 40-char commit SHA to create the branch from. Abbreviated SHAs are
    /// rejected by the GitHub API.
    pub from_sha: String,
}
...
impl KpGithubServer {
    ...
    #[rmcp::tool(name = "github_prs_mark_ready_for_review")]
    async fn github_prs_mark_ready_for_review(&self, Parameters(p): Parameters<PrDiffParams>) -> Result<CallToolResult, McpError> {
        let result = tools::prs::set_draft(&self.client, &p.owner, &p.repo, p.number, false).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        let output = self.compress_and_format(result, None, None);
        Ok(CallToolResult::success(vec![Content::text(output)]))
    }
    ...
    #[rmcp::tool(name = "github_prs_convert_to_draft")]
    async fn github_prs_convert_to_draft(&self, Parameters(p): Parameters<PrDiffParams>) -> Result<CallToolResult, McpError> {
        let result = tools::prs::set_draft(&self.client, &p.owner, &p.repo, p.number, true).await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?;
        let output = self.compress_and_format(result, None, None);
        Ok(CallToolResult::success(vec![Content::text(output)]))
    }
...
impl ServerHandler for KpGithubServer {
    // NOTE: `call_tool` / `list_tools` / `get_tool` are normally generated by
    // `#[rmcp::tool_handler]`. They are hand-written here so `call_tool` can
    // wrap the router dispatch in `GLOBAL_TOOL_TIMEOUT` — see the const above.
    // The MCP protocol itself has no server-side handler timeout (only a
    // client-side request timeout + `notifications/cancelled`), and rmcp adds
    // none, so the server must enforce its own backstop.
    async fn call_tool(
        &self,
...
mod tests {
    ...
    #[tokio::test]
    async fn test_tool_prs_mark_ready_for_review() {
        let server = mock_server(vec![
            json!({"number": 3, "node_id": "PR_abc"}),
            json!({"data": {"markPullRequestReadyForReview": {"pullRequest": {"number": 3, "isDraft": false}}}}),
        ]);
        let result = server.github_prs_mark_ready_for_review(Parameters(PrDiffParams {
            owner: "o".into(), repo: "r".into(), number: 3, max_bytes: None,
        })).await.unwrap();
        assert!(ok_text(&result).contains("3"));
    }

    #[tokio::test]
    async fn test_tool_prs_convert_to_draft() {
        let server = mock_server(vec![
            json!({"number": 3, "node_id": "PR_abc"}),
            json!({"data": {"convertPullRequestToDraft": {"pullRequest": {"number": 3, "isDraft": true}}}}),
        ]);
        let result = server.github_prs_convert_to_draft(Parameters(PrDiffParams {
            owner: "o".into(), repo: "r".into(), number: 3, max_bytes: None,
        })).await.unwrap();
        assert!(ok_text(&result).contains("isDraft"));
    }
