...
# case_sensitive: True means pattern relies on exact case (e.g. uppercase-only checks)
# case_sensitive: False (default) means search with re.IGNORECASE
COMPULSION_PATTERNS = [
...
def scan_file(path: Path) -> Iterator[Finding]:
    ...
    for i, line in enumerate(lines, 1):
        for hook in COMPULSION_PATTERNS:
            flags = 0 if hook.get("case_sensitive", False) else re.IGNORECASE
            if re.search(hook["pattern"], line, flags):
                yield Finding(
                    file=path,
                    line_num=i,
                    line=line.strip()[:80],
