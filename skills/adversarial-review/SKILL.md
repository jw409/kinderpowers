...
|-----------|---------|-------|-------------|
| `intensity` | medium | light, medium, full | Review depth — light for typos/config, medium for routine changes, full for security/architecture |
| `scope` | diff | diff, module, system | How wide to look — just the diff, the containing module, or cross-cutting system concerns |
| `auto_fix` | false | true/false | Whether to propose inline fixes or just report findings |
| `council_mode` | false | true/false | Spawn multi-perspective lenses (2-3x cost, 3-5x coverage) |
| `pedanticness` | medium | low, medium, high | What severity level gets raised — low=bugs only, high=includes nitpicks |
